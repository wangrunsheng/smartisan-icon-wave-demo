...
本分支保留三次贝塞尔实现，每半波一段曲线，控制柄比例为 0.36。
...
一个用 **Kotlin + Android Canvas** 复刻 **Smartisan OS 桌面图标下载水波纹动画**的实验 Demo。两条等波长的贝塞尔曲线以不同速度移动，分别裁切同一幅彩色图标，叠加出下载进度的水面效果。
...
每个半波使用一段三次贝塞尔曲线，水平控制柄长度为半波宽度的 `0.36`。曲线形状保持固定，只作匀速水平位移。前后波波长相同，但时间频率不同，因此相位差持续变化。
...
JVM 测试覆盖暂停恢复不跳时、慢放倍率、循环边界、波相位周期和水位范围。GitHub Actions 执行构建、测试及 Android Lint。UI 已在 Smartisan Android 8.1 真机检查；其他设备仍需实际验证。
